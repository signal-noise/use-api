...
- `pollInterval` - How often to re-request updated data. Pass 0 to disable polling (the default behavior).
- `payload` - A data object to send with the request. If we are performing a GET request, it is appended into the querystring (e.g. `?keywords=hello`). If it is a POST request it is sent in the request body as JSON.
